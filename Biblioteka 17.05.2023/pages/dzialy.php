...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo '<tr><td>' . $row['Id_dzial'] . '</td><td>'
            . $row['Nazwa'] . '</td><td><a href="?PAGE=dzialy_edytuj&id='.$row['Id_dzial'].'&Nazwa='.$row['Nazwa'].'">edycja </a></td><td><a href="?PAGE=dzialy_usun&id='.$row['Id_dzial'].'&Nazwa='.$row['Nazwa'].'">Usuń </a></td></tr>';
    }
    echo '</table>';
} else {
    echo 'brak danych';
